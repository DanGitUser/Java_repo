...
public class AccountService {
	...
	public void deposit() {
		 ...
		 account.setAmount(account.getAmount() + nextLong("입금할 금엑을 입력하세요"));
	}
	...
	public void withdraw() {
		List<Account> myAccounts = findAccountsBy(customerService.getLoginCustomer());
		 ...
		 long amount = nextLong("출금할 금엑을 입력하세요");
	...
	public void transfer() {
		List<Account> myAccounts = findAccountsBy(customerService.getLoginCustomer());
		 ...
		 long amount = nextLong("이체할 금엑을 입력하세요");
		 if (account.getAmount() < amount) {
			 System.out.println("잔고가 부족합니다");
			 return;
		 }
	...
	public void close() {
		List<Account> myAccounts = findAccountsBy(customerService.getLoginCustomer());
		 ...
		 if(nextConfirm("해지하시겠습니까? (남은 잔고 삭제 주의!) ")) {
			 return;
		 }
